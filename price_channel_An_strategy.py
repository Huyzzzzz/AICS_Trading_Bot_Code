...
def trendlines(dataframe, slicing_window=100, distance=50, chart=True, field_for_supports='low', field_for_resistances='high', timeframe="1h"):
    """
...
if __name__ == "__main__":
    ...
    """
    # Read data
    df_5m = pd.read_csv('/home/huy/user/freqtrade/huytq/test_12_price_channel/data_backtest/an.csv')[:900]
    ...
    end_time = "2024-01-01 18:00:00+00:00"
    ...
    # Add trendlines to the figure
    fig.add_trace(go.Scatter(x=x_values_date_time, y=y_values_max, mode='lines', name="Max Trendline"))
    fig.add_trace(go.Scatter(x=x_values_date_time, y=y_values_min, mode='lines', name="Min Trendline"))
    ...
    """
    length = 500
    resistance = df_5m['previous_nearest_resistance_1h'][end_indice]
    support = df_5m['previous_nearest_support_1h'][end_indice]
    bottom = df_5m['previous_nearest_bottom_5m'][end_indice]
    peak = df_5m['previous_nearest_peak_5m'][end_indice]
    print(resistance, support, bottom, peak)

    x = np.arange(end_indice + 1 - length, end_indice + 1, 1)
    time_range = df_5m.loc[x, 'date']
